...
from config.settings import DRAFT_COUNT
...
def test_no_new_notes_falls_back_to_pillars():
    ...
    assert len(rows) == DRAFT_COUNT
    assert len(sheets.queue_rows()) == DRAFT_COUNT
    _, user_content = claude.calls[0]
    assert "小言なし" in user_content


def test_insufficient_notes_uses_all_new_and_backfills():
    # 2 new notes, DRAFT_COUNT (7) drafts -> 2 used, rest pillar-backfilled
    sheets = FakeSheets(
        [
    ...
    _, user_content = claude.calls[0]
    # instructs to backfill the remaining 5
    assert "残り5本" in user_content
...
def test_more_notes_than_count_marks_only_used_slice():
    ...
    assert used == DRAFT_COUNT  # only the prioritized slice
    assert new == 9 - DRAFT_COUNT  # leftovers stay new for next run
